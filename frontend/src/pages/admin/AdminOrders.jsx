...
import { Search, Eye, ChevronLeft, ChevronRight, Clock, Check, Truck, XCircle } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
const AdminOrders = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium ${config.class}`}
      >
            ...
            {selectedOrder && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h4 className="mb-2 font-semibold">Lieferadresse</h4>
                    <p>
                      {selectedOrder.shipping_info?.first_name}{' '}
                      {selectedOrder.shipping_info?.last_name}
                    </p>
                    <p>
                      {selectedOrder.shipping_info?.address?.street}{' '}
                      {selectedOrder.shipping_info?.address?.house_number}
                    </p>
                    <p>
                      {selectedOrder.shipping_info?.address?.postal_code}{' '}
                      {selectedOrder.shipping_info?.address?.city}
                    </p>
                  </div>
                  <div>
                ...
                <div>
                  <h4 className="mb-2 font-semibold">Bestellte Artikel</h4>
                  <div className="space-y-2">
                    {selectedOrder.items?.map((item, i) => (
                      <div key={i} className="flex items-center gap-3 rounded bg-gray-50 p-2">
                        <img

...
const AdminWorkshop = () => {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      await axios.put(
        `${API}/workshop/appointments/${id}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      fetchAppointments()
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button className="bg-[#4fd1c5] text-[#1e3a5f] hover:bg-[#38b2ac]">Neuer Termin</Button>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (
              ...
              <p className="py-8 text-center text-gray-500">Keine Termine vorhanden.</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                      ...
                      <TableCell>
                        <div className="flex flex-col text-sm">
                          ...
                          <span className="flex items-center gap-1 text-gray-500">
                            <Clock className="h-3 w-3" /> {apt.phone}
                          </span>
                        </div>
                      </TableCell>

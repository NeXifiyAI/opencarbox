...
export class Memory {
  ...
  static async getStats(): Promise<{
    ...
    return this.withRetry(
      async () => {
        const { data, error } = await getSupabase().from('project_memory').select('type')
  ...
  static async cleanup(
    ...
    keepTypes: MemoryType[] = ['BEST_PRACTICE']
  ): Promise<number> {
    return this.withRetry(
      async () => {

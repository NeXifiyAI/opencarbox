...
4. nextTask sollte IMMER gefüllt sein wenn sinnvoll`
  ...
  static async endSession(): Promise<void> {
    ...
    if (state.session) {
      ...
      console.log(
        `👋 Session beendet: ${state.session.id} (${state.session.tasksCompleted} Tasks in ${Math.round(duration / 1000 / 60)}min)`
      )
  ...
  private static async loadAllMemories(): Promise<string> {
    try {
      ...
      if (todos.length > 0) {
        sections.push(
          '### OFFENE TODOS\n' +
            todos.map((m: any) => `📌 ${m.title}: ${m.content?.substring(0, 150)}`).join('\n')
        )
      }
      ...
      if (knowledge.length > 0) {
        sections.push(
          '### WISSEN\n' +
            knowledge.map((m: any) => `📚 ${m.title}: ${m.content?.substring(0, 150)}`).join('\n')
        )
      }
...
Gib konkrete Präventionsmaßnahmen an, um diese Fehler in Zukunft zu vermeiden.
  ...
  private static async loadPendingTasks(): Promise<string> {
    ...
    if (state.taskQueue.length > 0) {
      ...
      sections.push(
        state.taskQueue.map((t) => `- [${t.priority}] ${t.description} (${t.status})`).join('\n')
      )
    }
    ...
    try {
      ...
      if (todos.length > 0) {
        ...
        sections.push(todos.map((t: any) => `- 📌 ${t.title}`).join('\n'))
      }
    } catch {
      // Silent fail
    }
    ...
    if (state.taskHistory.length > 0) {
      ...
      sections.push(
        recent.map((t) => `- ${t.status === 'COMPLETED' ? '✅' : '❌'} ${t.description}`).join('\n')
      )
    }
  ...
  private static async documentInteraction(
    prompt: string,
  ...
  private static async withRetry<T>(fn: () => Promise<T>, operationName: string): Promise<T> {
    ...
    for (let attempt = 1; attempt <= ORACLE_CONFIG.maxRetries; attempt++) {
      try {
        ...
        console.error(
          `${operationName}: Versuch ${attempt}/${ORACLE_CONFIG.maxRetries} fehlgeschlagen:`,
          lastError.message
        )
  ...
  ;(async () => {
    switch (command) {
      ...
      case 'snapshot':
        ...
        console.log(
          `Health: API=${stat.health.oracle ? '✅' : '❌'} Memory=${stat.health.memory ? '✅' : '❌'} Context=${stat.health.context ? '✅' : '❌'}`
        )
        break

      case 'learn':
        const learnType =
          (args[1] as 'success' | 'failure' | 'pattern' | 'knowledge') || 'knowledge'
      ...
      case 'session':
        ...
        if (sessionCmd === 'start') {
          ...
          console.log(
            `Session: ${s.session ? JSON.stringify(s.session, null, 2) : 'Keine aktive Session'}`
          )
        }
        break

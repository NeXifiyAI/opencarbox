...
const cardVariants = cva('rounded-xl transition-all duration-200', {
  variants: {
    variant: {
      ...
      carvantooo:
        'border border-carvantooo-200 bg-white shadow-sm hover:shadow-md hover:border-carvantooo-300',
      opencarbox:
        'border border-opencarbox-200 bg-white shadow-sm hover:shadow-md hover:border-opencarbox-300',
...
const CardImage = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('relative aspect-[4/3] overflow-hidden rounded-t-xl', className)}
      {...props}
    />
  )
)
